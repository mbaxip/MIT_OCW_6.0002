...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        return Edge.__str__(self) + " (" + str(self.total_distance) + ", " + str(self.outdoor_distance) + ")"
...
class Digraph(object):
    ...
    def add_node(self, node):
        ...
        if node in self.nodes:
            raise ValueError('Node already exists in the Digraph')
        else:
            self.nodes.add(node)
            self.edges[node] = []
    ...
    def add_edge(self, edge):
        ...
        if edge.get_source() in self.nodes and edge.get_destination() in self.nodes:
            ...
            raise ValueError('Source or Destination Node does not does not exist in the Digraph')
