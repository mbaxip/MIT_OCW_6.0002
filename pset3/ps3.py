...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        if capacity > self.tiles[(int(pos.get_x()), int(pos.get_y()))]:
            self.tiles[(int(pos.get_x()), int(pos.get_y()))] = 0
        else:
            self.tiles[(int(pos.get_x()), int(pos.get_y()))] = self.tiles[(int(pos.get_x()), int(pos.get_y()))] - capacity
    ...
    def get_num_cleaned_tiles(self):
        ...
        for key in self.tiles.keys():
            if self.tiles[key] == 0:
                num_clean_tiles = num_clean_tiles + 1
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        """
        if(int(pos.get_x()), int(pos.get_y())) in self.furniture_tiles:
            return True
        else:
            return False
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.room.is_position_valid(new_position):
            # Move the robot to a new random position
            self.set_robot_position(new_position)
            # clean the dirt on the tile by its given capacity.
            self.room.clean_tile_at_position(self.position, self.capacity)
        # If the new position is invalid
        else:
            # Rotate once to a random new direction, and stay stationary
...
class RandomWalkRobot(Robot):
    """
    A RandomWalkRobot is a Robot with new movement strategy.

    At each time-step, a StandardRobot chooses a new direction randomly.
    """
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.gets_faulty():
            # do not clean the current tile and change its direction randomly.
            self.set_robot_direction(random.uniform(0,360))
        # If the robot does not get faulty the robot should behave like StandardRobot
        else:
            new_position = self.position.get_new_position(self.direction, self.speed)
            # Check if the new position is valid
            if self.room.is_position_valid(new_position):
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for i in range(num_trials):
        ...
        if is_furnished:
            ...
            # Visualization
            # anim = ps3_visualize.RobotVisualization(num_robots, width, height, room.furniture_tiles, delay=0.2)
        else:
            room = EmptyRoom(width, height, dirt_amount)
            # Visualization
            # anim = ps3_visualize.RobotVisualization(num_robots, width, height, [], delay=0.2)
        ...
        trial_time_steps.append(time_steps)
    mean_time_steps = sum(trial_time_steps)/len(trial_time_steps)
    return mean_time_steps


# print ('avg time steps: ' + str(run_simulation(5, 1.0, 1, 5, 5, 3, 1.0, 1, RandomWalkRobot)))
...
def show_plot_compare_strategies(title, x_label, y_label):
    ...
    for num_robots in num_robot_range:
        ...
        times3.append(run_simulation(num_robots, 1.0, 1, 20, 20, 3, 0.8, 20, RandomWalkRobot))
    pylab.plot(num_robot_range, times1)
    pylab.plot(num_robot_range, times2)
    pylab.plot(num_robot_range, times3)
    pylab.title(title)
    pylab.legend(('StandardRobot', 'FaultyRobot', 'RandomWalkRobot'))
